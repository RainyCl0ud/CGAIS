...
<body class="min-h-screen min-w-full bg-gradient-to-br from-yellow-100 via-white to-blue-100 overflow-hidden">
    <div class="flex flex-col items-center justify-center min-h-screen w-full">
        <div class="w-full max-w-2xl mx-auto p-16 rounded-2xl shadow-2xl bg-white/90 border border-blue-100">
            <div class="flex flex-col items-center">
                ...
                <p class="text-xl text-gray-600 text-center mb-10">Website still on progress...</p>
                <div class="flex gap-8 mb-2">
                    @if (Route::has('login'))
                        @auth
                            <a href="{{ url('/dashboard') }}" class="px-10 py-4 text-lg bg-[#FFD700] hover:bg-[#FFE44D] text-[#1E3A8A] font-bold rounded-lg shadow transition-colors border border-blue-200">Dashboard</a>
                        @else
                            <a href="{{ route('login') }}" class="px-10 py-4 text-lg bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow transition-colors border border-blue-700">Log in</a>
                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="px-10 py-4 text-lg bg-[#FFD700] hover:bg-[#FFE44D] text-[#1E3A8A] font-bold rounded-lg shadow transition-colors border border-blue-200">Register</a>
                            @endif
                        @endauth
                    @endif
                </div>

            ...
            <div class="w-full max-w-7xl mx-auto">
                <div class="bg-white/80 rounded-lg sm:rounded-2xl shadow-lg sm:shadow-2xl border border-blue-100 p-4 sm:p-8 backdrop-blur">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6">
                        ...
                        <p class="text-gray-600 text-xs sm:text-sm mt-1">View your session history</p>
                        </div>
                    <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 mt-3 sm:mt-0">
                        {{-- <a href="{{ route('appointments.index') }}" 
                           class="px-4 sm:px-6 py-2 sm:py-3 bg-blue-900 text-white font-semibold rounded-lg hover:bg-blue-800 transition-colors text-sm">
                            ...
                            <a href="{{ route('appointments.export-history') }}?{{ request()->getQueryString() }}" 
                    ...
                    <div class="bg-gray-50 p-4 sm:p-6 rounded-lg mb-6">
                        ...
                        <form method="GET" action="{{ route('appointments.session-history') }}" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <!-- Search -->
                            <div>
                                <label for="search" class="block text-sm font-medium text-gray-700 mb-1">Search</label>
                                <input type="text" name="search" id="search" value="{{ request('search') }}"
                                   placeholder="Student or faculty name or ID..."
                            ...
                            <!-- Filter Buttons -->
                            <div class="col-span-full flex flex-row gap-2 justify-start sm:justify-end">
                                <button type="submit" class="px-4 py-2 bg-blue-900 text-white font-semibold rounded-lg hover:bg-blue-800 transition-colors">
                                    🔍 Apply Filters
                                </button>
                                <a href="{{ route('appointments.session-history') }}" class="px-4 py-2 bg-gray-500 text-white font-semibold rounded-lg hover:bg-gray-600 transition-colors text-center">
                                    🗑️ Clear Filters
                                </a>
                            </div>
                        </form>

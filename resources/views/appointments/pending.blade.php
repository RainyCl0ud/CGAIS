                ...
                <div class="bg-white/80 rounded-lg sm:rounded-2xl shadow-lg sm:shadow-2xl border border-blue-100 p-4 sm:p-8 backdrop-blur">
                    <div class="mb-4 sm:mb-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-blue-900">Pending Appointments</h1>
                    ...
                    @if($pendingAppointments->count() > 0)
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach($pendingAppointments as $appointment)
                                            <tr class="hover:bg-gray-50 cursor-pointer" onclick="window.location.href='{{ route('appointments.show', $appointment) }}?back=pending'">
                                                <td class="px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">
                                                    <div class="font-medium">{{ $appointment->getFormattedDateTime() }}</div>
                                                    <div class="text-gray-500 text-xs">{{ $appointment->appointment_date->diffForHumans() }}</div>
                                                </td>
                                                <td class="px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">
                                                ...
                                                <td class="px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap">
                                                    ...
                                                    @elseif($appointment->isReferral())
                                                        <div class="text-xs text-yellow-600 font-medium mt-1">↪ Referral</div>
                                                    @endif
                                                </td>
                                                <td class="px-3 sm:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">
                        ...
                        <div class="mt-4 text-center text-sm text-gray-600">
                            <p>Showing {{ $pendingAppointments->count() }} pending appointment(s)</p>
                    ...
                    @else
                        <div class="text-center py-8 sm:py-12">
                            ...
                            <h3 class="mt-2 text-sm font-medium text-gray-900">No pending appointments</h3>

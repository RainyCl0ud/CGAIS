<x-app-layout>
    <div class="flex flex-col items-center justify-start py-4 sm:py-8 px-2 sm:px-4">
        <div class="w-full max-w-2xl mx-auto">
            <div class="bg-white/80 rounded-lg sm:rounded-2xl shadow-lg sm:shadow-2xl border border-blue-100 p-4 sm:p-8 backdrop-blur">
                ...
                <form method="POST" action="{{ route('appointments.update', $appointment) }}" class="space-y-4 sm:space-y-6">
                    @csrf
                    @method('PUT')
    ...
    <script>
        ...
        function updateAvailableSlots() {
            ...
            fetch(`/api/counselors/${counselorId}/available-slots?date=${encodeURIComponent(date)}&urgent=${isUrgent ? 1 : 0}&exclude_appointment_id=${excludeAppointmentId}`)
